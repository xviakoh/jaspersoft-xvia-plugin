...
public class TrackViaCreator implements IDataAdapterCreator {

	@Override
	public DataAdapterDescriptor buildFromXML(Document docXML) {
		TrackViaDataAdapterImpl result = new TrackViaDataAdapterImpl();
		...
		for(int i=0; i<children.getLength(); i++){
			...
			if (node.getNodeName().equals("connectionParameter")){
				...
				if (paramName.equals("TrackVia URI")) result.setTrackViaURI(node.getTextContent());
				if (paramName.equals("password")) result.setPassword(node.getTextContent());
			}
		}

		TrackViaDataAdapterDescriptor desc = new TrackViaDataAdapterDescriptor();
	...
	@Override
	public String getID() {
		return "com.jaspersoft.ireport.xvia.connection.TrackViaConnection";
	}
